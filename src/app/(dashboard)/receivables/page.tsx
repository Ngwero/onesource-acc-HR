...
export default function ReceivablesPage() {
  ...
  const load = useCallback(() => {
    fetch("/api/receivables?limit=100").then((r) => r.json()).then((res) => {
      if (res.success) {
  ...
  return (
    <div>
      ...
      {collectTarget && (
        <FormModal title={`Collect from ${String((collectTarget.customer as { name?: string })?.name || "Customer")}`} open onOpenChange={(o) => { if (!o) setCollectTarget(null); }}>
          {({ close }) => (
            <form onSubmit={(e) => { e.preventDefault(); handleCollect(close); }} className="space-y-3">
              <p className="text-sm">Balance: {formatCurrency(Number(collectTarget.balance))}</p>
              <FormField label="Collection Amount">
                <Input type="number" value={amount} onChange={(e) => setAmount(+e.target.value)} required max={Number(collectTarget.balance)} />
              </FormField>
      ...
      <DataTable columns={[
        ...
        { key: "id", header: "Action", render: (i) => ["UNPAID", "PARTIALLY_PAID", "OVERDUE"].includes(String(i.status)) ? (
          <Button size="sm" onClick={() => { setCollectTarget(i); setAmount(Number(i.balance)); }}>Collect</Button>
        ) : null },
      ]} data={items} />
    </div>
  );
